...
MARKER = "LTM_CONTACT_OVERLAY_v3"
OLD_MARKERS = ("LTM_CONTACT_OVERLAY_v1", "LTM_CONTACT_OVERLAY_v2", "LTM_CONTACT_OVERLAY_v3")
...
(function () {{
  ...
  var css = ""
    ...
    /* Feedback pill — bottom-left, next to disclaimer chip */
    + ".ltm-cu-fb{{position:fixed;left:14px;bottom:56px;z-index:2147482998;"
    + "background:#fff;color:#141414;border:1px solid #d9d9d9;cursor:pointer;"
    + "font:600 11px/1 Inter,-apple-system,sans-serif;letter-spacing:.08em;text-transform:uppercase;"
    + "padding:8px 11px;border-radius:999px;box-shadow:0 2px 8px rgba(0,0,0,.10);"
    + "display:inline-flex;align-items:center;gap:6px}}"
    + ".ltm-cu-fb:hover{{background:#f4f4f3;border-color:#ff5e4f;color:#e5412f}}"
    + ".ltm-cu-fb .ltm-cu-fb-icon{{font-size:12px;line-height:1}}"
    ...
    + "}}"
    + "@media print{{"
    +   ".ltm-cu-tab,.ltm-cu-chip,.ltm-cu-pop,.ltm-cu-modal,.ltm-cu-fb{{display:none !important}}"
    + "}}";
  ...
  function inject() {{
    ...
    /* Feedback pill (bottom-left, above the disclaimer chip). Opens a
       fresh mailto to Ashutosh with framework + page URL prefilled. No
       cookies, no tracking — user reviews and sends from their mail app. */
    var fb = document.createElement("button");
    fb.id = "ltm-cu-fb"; fb.className = "ltm-cu-fb"; fb.type = "button";
    fb.setAttribute("aria-label", "Send feedback about this assessment");
    fb.innerHTML = "<span class='ltm-cu-fb-icon'>✎</span>Feedback";
    fb.onclick = function(){{
      var lines = [
        "Hi Ashutosh,","",
        "Feedback on the " + CFG.accelerator + " (" + CFG.framework + "):",
      ...
      var subject = CFG.accelerator + " — feedback (" + CFG.framework + ")";
